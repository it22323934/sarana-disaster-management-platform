"""incident-svc — intake, verification, triage, dispatch.
...
Channels, the state machine, and the dispatch gate are docs/build-prompts/08-incident-service.md's job.
"""
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(title="SARANA incident-svc", lifespan=lifespan)
    app.state.app_state = state

    register_exception_handlers(app)
    app.include_router(make_health_router(state))
    # api/v1 routers are mounted here once docs/build-prompts/08-incident-service.md builds them.
