...
def _public_ref(division_code: str) -> str:
    """A short human reference an operator can read over a radio."""
    suffix = str(uuid7())[-6:].upper()
    return f"INC-{division_code}-{suffix}"
...
async def accept(
    ...
    *,
    ...
    report = await queries.insert_report(
        ...
        lon=intake.lon,
        lat=intake.lat,
        location_accuracy_m=intake.location_accuracy_m,
        location_source=intake.location_source,
    ...
    division = None
    if intake.has_location:
        division = await core_api.resolve(
            lon=float(intake.lon or 0), lat=float(intake.lat or 0), token=token
        )
    ...
    if division is None:
        _log.info(
            ...
            channel=intake.channel,
            had_location=intake.has_location,
        )
        await _enqueue_received(session, intake, report_id, placed=False)
        return IntakeResult(
            report_id=report["id"],
            incident_id=None,
            public_ref=None,
    ...
    if candidates:
        ...
        incident = await queries.insert_incident(
            session,
            public_ref=_public_ref(division.gn_division_code),
    ...
    await _enqueue_received(session, intake, report_id, placed=True, incident_id=incident_id)
    ...
    *,
    ...
    envelope = EventEnvelope(
        ...
        payload={
            ...
            "channel": intake.channel,
            "placed": placed,
            "has_location": intake.has_location,
        },
    )
