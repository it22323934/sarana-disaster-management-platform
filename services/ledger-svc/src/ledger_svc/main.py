"""ledger-svc — assessments, entitlements, approvals, disbursement, grievances.
...
Entitlement calculation, the hash chain, and the disbursement gate are
docs/build-prompts/10-aid-ledger-service.md's job.
"""
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(title="SARANA ledger-svc", lifespan=lifespan)
    app.state.app_state = state

    register_exception_handlers(app)
    app.include_router(make_health_router(state))
    # api/v1 routers are mounted here once docs/build-prompts/10-aid-ledger-service.md builds them.
