"""agent-svc — the six agents and the shared LangGraph runtime.
...
The LangGraph runtime (model routing, checkpointing, reusable nodes) is
docs/build-prompts/12-langgraph-runtime.md's job; the agents themselves are 13-18.
"""
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(title="SARANA agent-svc", lifespan=lifespan)
    app.state.app_state = state

    register_exception_handlers(app)
    app.include_router(make_health_router(state))
    # api/v1/agents/... is mounted here once docs/build-prompts/12-langgraph-runtime.md builds it.
