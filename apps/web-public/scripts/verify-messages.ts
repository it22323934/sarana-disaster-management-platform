/**
 * The public dashboard's i18n gate. `pnpm --filter @sarana/web-public verify-i18n`.
 *
...
/**
 * Where a namespace gets bound to a name.
 *
 * Two forms, not one, and the second is why this file is not a straight copy of the
 * console's. Every page on this site is a server component, so it binds with
 * `const t = await getTranslations({ locale, namespace: 'home' })` rather than
 * `useTranslations('home')`. A regex that only knew the hook form would resolve nothing
 * here, report zero checked keys, and pass — which is the worst kind of green.
 */
const BINDING =
...
function usagesIn(file: string): { usages: Usage[]; dynamic: number } {
  ...
  for (const match of source.matchAll(BINDING)) {
    bindings.push({
      at: match.index ?? 0,
      name: match[1] ?? '',
      // Group 2 is the hook's positional argument, group 3 the server form's `namespace`
      // key. Exactly one of them matches per binding.
...
function main(): void {
  ...
  if (!result.complete) {
    console.error(
      `apps/web-public/messages: ${result.problems.length} problem(s) across ${result.keyCount} keys\n`,
    );
  ...
  if (missing.length > 0) {
    console.error(
      `apps/web-public: ${missing.length} message key(s) used in code and absent from the catalogue.\n` +
  ...
  console.log(
    `apps/web-public/messages: ${result.keyCount} keys complete in si, ta and en; ` +
